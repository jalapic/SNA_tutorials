...
# Function to lexicographically sort each intransitive out-degree set
sort_intransitive_entries <- function(degree_string) {
  if (degree_string == "") return("")  # Handle empty case
  triads <- unlist(strsplit(degree_string, " \\| "))  # Split on " | "
  sorted_triads <- sort(triads)  # Sort lexicographically
  return(paste(sorted_triads, collapse=" | "))  # Reassemble
}

# Apply the sorting function to the column
tournament_data <- tournament_data %>%
  mutate(Intransitive_Out_Degrees = sapply(Intransitive_Out_Degrees, sort_intransitive_entries))
...
#### Do for n=3, n=4, n=5 nodes.


#### --------------------------- N=3 --------------------------------

# Generate all tournaments for n=3
all_tournaments <- generate_tournament_graphs(3)

# Filter to get n non-isomorphic tournament graphs
unique_tournaments <- get_non_isomorphic_tournaments(all_tournaments)
...
# Compute properties for all n tournaments
tournament_data <- data.frame()
...
# Reorder the tournament numbering 1:n
tournament_data$Tournament <- 1:nrow(tournament_data)
...
# Generate all tournaments for n=4
all_tournaments <- generate_tournament_graphs(4)
...
# Generate all tournaments for n=5
all_tournaments <- generate_tournament_graphs(5)
