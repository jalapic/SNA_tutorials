...
# find how many interactions happened in each state - uses clean_dfs2
all_isomorphs <- sort(unique(unlist(lapply(clean_dfs2, \(x) x$isomorph_class))))

state_counts <- map_dfr(seq_along(clean_dfs2), function(i) {
  clean_dfs2[[i]] %>%
