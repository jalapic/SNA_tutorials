...
TM_list <- vector("list", length(clean_dfs2))

for (i in seq_along(clean_dfs2)) {
  TM_list[[i]] <- make_TM(clean_dfs2[[i]])
}
...
for (i in 1:5) {
  heatmaps[[i]] <- ggplot(TM_diffs_long[[i]], aes(x = Column, y = Row, fill = Value)) +
    ...
    labs(title = glue("Time-Dependent Cohort {i} Diff from Random Walk"),
         x = "Next State",
         y = "Current State",
         fill = "Difference") +
    theme_minimal() +
...
for (i in 6:9) {
  heatmaps[[i]] <- ggplot(TM_diffs_long[[i]], aes(x = Column, y = Row, fill = Value)) +
    geom_tile(color = "black") +  # Add tile borders
    scale_fill_gradient2(low = "darkcyan", mid = "white", high = "orangered") +
    labs(title = glue("Time-Dependent Cohort {i+1} Diff from Random Walk"),
         x = "Next State",
         y = "Current State",
         fill = "Difference") +
    theme_minimal() +
